...
import { Plus, Search, Edit3, XCircle, Globe } from 'lucide-react'
import { generateId } from '@/lib/utils'
import { pushChange } from '@/lib/editorRef'

const typeLabels: Record<string, string> = {
  location: '地点', organization: '组织', rule: '规则', item: '物品', other: '其他',
}
...
export function WorldPanel() {
  ...
  const [editing, setEditing] = useState<WorldCard | null>(null)
  const [ctxMenu, setCtxMenu] = useState<{ x: number; y: number; card: WorldCard } | null>(null)
  ...
  const load = async () => {
    ...
    const list = await window.electronAPI.world.list(activeProjectId)
  ...
  const handleDelete = async (id: string) => {
    ...
    await window.electronAPI.world.delete(id)
    load()
    if (activeProjectId && card) pushChange(activeProjectId, 'world', id, `世界观删除：${card.name}`)
  }
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-1 border-b pr-1 pl-3 py-1.5">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索设定..." className="h-7 pl-6 text-xs" />
        </div>
      ...
      <div className="flex gap-1 border-b px-3 py-1 overflow-x-auto">
        <button onClick={() => setFilterType('')} className={`text-[10px] rounded px-1.5 py-0.5 shrink-0 ${!filterType ? 'bg-accent text-accent-foreground' : 'hover:bg-accent/50'}`}>全部</button>
        {Object.entries(typeLabels).map(([k, v]) => (
          <button key={k} onClick={() => setFilterType(k)} className={`text-[10px] rounded px-1.5 py-0.5 shrink-0 ${filterType === k ? 'bg-accent text-accent-foreground' : 'hover:bg-accent/50'}`}>{v}</button>
        ))}
      </div>
        ...
        <div className="p-2 pb-10 space-y-1">
            ...
            <div key={card.id} className="rounded border px-3 py-2 hover:bg-sidebar-accent/50 cursor-pointer" onClick={() => { setEditing(card); setEditorOpen(true) }} onContextMenu={(e) => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY, card }) }}>
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-muted-foreground">{typeLabels[card.card_type] || card.card_type}</div>
                </div>
              </div>
              {card.description && <div className="text-[10px] text-muted-foreground mt-1 line-clamp-2">{card.description}</div>}
              {(card.tags || []).length > 0 && (
                <div className="flex gap-1 mt-1 flex-wrap">
                  {(card.tags || []).map((t) => <span key={t} className="text-[9px] bg-secondary text-secondary-foreground rounded px-1 py-0.5">{t}</span>)}
                </div>
              )}
            </div>
          ))}
          {filtered.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">
              {cards.length === 0 ? '暂无设定，点击 + 添加' : '未找到匹配设定'}
            </p>
          )}
        </div>
      </ScrollArea>
      ...
      {activeProjectId && (
        <WorldCardEditor open={editorOpen} onOpenChange={setEditorOpen} projectId={activeProjectId} card={editing} onSaved={load} />
      )}
    </div>
  )
}
